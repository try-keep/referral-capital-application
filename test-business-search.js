...
} = require('./src/lib/businessRegistry.ts');
...
async function testBusinessSearch() {
  ...
  try {
    ...
    if (results1.docs.length > 0) {
      ...
      console.log(
        `   • Location: ${business.City}, ${business.Reg_office_province}`
      );
    ...
    if (results2.docs.length > 0) {
      ...
      results2.docs.slice(0, 3).forEach((business, index) => {
        console.log(
          `   ${index + 1}. ${business.Company_Name} (${business.City}, ${business.Reg_office_province})`
        );
      });
    }
    ...
    const results3 = await searchCanadianBusinesses(
      'NONEXISTENT COMPANY XYZ123'
    );

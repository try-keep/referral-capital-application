...
const testCases = {
  ...
  aiCategorization: [
    ...
    { businessName: 'TechSoft Solutions', expectedCategory: 'Technology and Software' },
    { businessName: 'Green Construction LLC', expectedCategory: 'Construction and Real Estate' },
    { businessName: 'Bitcoin Trading Platform', expectedCategory: 'Cryptocurrency' }, // High-risk
  ],
  adverseMedia: [
  ...
  for (const testCase of testCases.website.valid) {
    ...
    if (result.error) {
      ...
      if (result.result.metadata) {
        ...
        console.log(`   Emails found: ${result.result.metadata.emails?.length || 0}`);
        console.log(`   Social media: ${Object.keys(result.result.metadata.social_media_links || {}).length} platforms`);
      }
    } else {
...
async function testAICategorization() {
  ...
  for (const testCase of testCases.aiCategorization) {
    ...
    if (result.error) {
      ...
      console.log(`✅ Success: ${aiResult.category} (confidence: ${aiResult.confidence_score})`);
      console.log(`   Summary: ${aiResult.summary}`);
      if (aiResult.high_risk_similarity > 0) {
        console.log(`   ⚠️  High-risk similarity: ${aiResult.high_risk_similarity}`);
      }
    } else {
      console.log(`❓ Unexpected response: ${JSON.stringify(result)}`);
    }
...
async function testAdverseMedia() {
  ...
  for (const testCase of testCases.adverseMedia) {
    ...
    if (result.error) {
      ...
      console.log(`   Overall sentiment: ${mediaResult.overall_sentiment_score} (${mediaResult.risk_level} risk)`);
  ...
  if (results.error) {
    ...
    if (results.checks && results.checks.length > 0) {
      ...
      results.checks.slice(0, 5).forEach((check, index) => {
        console.log(`${index + 1}. Type: ${check.check_type}, Status: ${check.status}, Risk: ${check.risk_score || 'N/A'}`);
      });
    }
  } else {
...
async function runTests() {
  ...
  try {
    ...
    console.log('- Website: curl -X POST ' + BASE_URL + '/api/test-compliance -H "Content-Type: application/json" -d \'{"type": "website", "websiteUrl": "google.com", "businessName": "Google"}\'');
    console.log('- AI: curl -X POST ' + BASE_URL + '/api/test-compliance -H "Content-Type: application/json" -d \'{"type": "categorization", "businessName": "ABC Restaurant"}\'');
    console.log('- Media: curl -X POST ' + BASE_URL + '/api/test-compliance -H "Content-Type: application/json" -d \'{"type": "adverse-media", "businessName": "Microsoft"}\'');

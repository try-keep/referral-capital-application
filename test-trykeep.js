...
async function testTryKeepCompliance() {
    ...
    try {
      ...
      if (responseJson.success) {
        ...
        if (responseJson.metadata) {
          ...
          console.log(
            `Website Accessible: ${responseJson.metadata.success ? 'Yes' : 'No'}`
          );
          console.log(
            `Phone Numbers Found: ${responseJson.metadata.phone_numbers?.length || 0}`
          );
          console.log(
            `Emails Found: ${responseJson.metadata.emails?.length || 0}`
          );
          console.log(
            `Social Media Platforms: ${Object.keys(responseJson.metadata.social_media_links || {}).length}`
          );
        }
      } else {
  ...
  try {
    ...
    const missingKeys = [];
    if (!debugData.env_check.supabase_service_key)

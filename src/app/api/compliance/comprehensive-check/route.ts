...
async function searchNewsAPI(
  ...
  console.log(
    'NewsAPI URL (without key):',
    url.replace(newsApiKey, '[REDACTED]')
  );
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      'Processing comprehensive compliance check for:',
      businessWebsite
    );
    ...
    try {
      ...
      const metadata: WebsiteMetadata =
        await scrapeWebsiteMetadata(businessWebsite);
      ...
      if (Object.keys(metadata.social_media_links).length > 0)
      ...
      return NextResponse.json(
        {
          error: 'Failed to process comprehensive compliance check',

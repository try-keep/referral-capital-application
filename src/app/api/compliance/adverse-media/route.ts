...
async function searchNewsAPI(businessName: string, businessUrl?: string): Promise<NewsArticle[]> {
  if (!NEWSAPI_KEY) {
...
async function analyzeSentimentWithOpenAI(articleText: string): Promise<number> {
  if (!OPENAI_API_KEY) {
...
async function runAdverseMediaCheck(businessName: string, businessUrl?: string): Promise<AdverseMediaResult> {
  try {
    ...
    for (const article of articles.slice(0, 5)) { // Limit to 5 articles to control costs
      try {
    ...
    const overallSentiment = analyzedArticles.length > 0 
    ...
    throw new Error(`Adverse media check failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      return NextResponse.json(
        { error: 'Failed to process adverse media check', details: error instanceof Error ? error.message : 'Unknown error' },
        { status: 500 }
      );
    }
